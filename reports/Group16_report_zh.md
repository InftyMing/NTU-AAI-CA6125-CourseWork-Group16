...
| 成员 | 主要分工 | 贡献 |
| :--- | :--- | :--- |
| DHAYALA MURTHY SAVITHACCDS | 文献调研与问题定义 | 调研 Feedback Prize 公开方案与 AES 相关研究，整理 §3、§8 引用的 Qwen / LoRA / DeBERTa-v3 等参考文献，主导第 1、3、8 节。 |
| HUANG ZIXUANCCDS | 数据审计与特征工程 | 实现数据审计模块、十个文本统计量、TF-IDF + SVD 特征流水线，以及 LLM/RAG 探针所需的验证切片。 |
| SUN MINGCCDS | 建模与评估 | 实现 MCRMSE、交叉验证框架、所有传统成分模型与堆叠器，并完成 §7.7 Qwen2.5-1.5B + LoRA 微调与 §7.8 DeBERTa-v3-base RAG 检索器。 |
| TAN XUAN ZHAOCCDS | 前端演示与部署 | 构建 FastAPI 服务、SVG 前端（含新增的“LLM and RAG”面板）、Docker 镜像与阿里云部署。 |
| WANG QIANCCDS | 报告、视频与协调 | 整合实验结果，撰写中英文报告与视频脚本，并把 LLM/RAG 探针纳入与课程主题对齐的叙述。 |

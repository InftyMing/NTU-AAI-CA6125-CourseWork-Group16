...
公开的 Kaggle Ridge 与 LightGBM 单模型方案 MCRMSE 大致落在 0.49–0.52（Sancho, 2022；Daniel Gaddam, 2022；FEATURESELECTION, 2022）。我们的 0.51873 处于该区间内，且仅依赖 scikit-learn 标准组件、不需要 GPU。第一名方案（Singh & Maslov, 2022）通过堆叠数十个 DeBERTa-v3-large、配合 RAPIDS SVR 头实现 0.43。第 8 节会基于实测数据解释为什么我们没有去追这个数字；§7.7 与 §7.8 进一步给出两条 Notebook 形式的 LLM/RAG 探针，以正面回应课程主题。

### 7.7 生成式 LLM 微调（Qwen2.5-1.5B + LoRA）

课程的题目是 *LLM and RAG*，因此我们至少要有一组实验让生成式语言模型直接产出分维度评分。我们选择 **Qwen2.5-1.5B-Instruct** 作为底座——它体量适中、可在 Apple Silicon 笔记本（16 GB 统一内存、MPS、fp16）上完整训练，并且经过指令微调、自带 chat template。训练 Notebook 位于 `LLM experiments/local training_essay_scoring_m_chip.ipynb`，对应的 Kaggle 推理 Notebook 在 `LLM experiments/kaggle inference_group-16.ipynb`。

**实验设置。** 用 Qwen 的 chat 模板对每篇作文进行分词，使用固定指令（*"Rate this English essay on 6 dimensions (1.0–5.0 in 0.5 steps). Output ONLY JSON: {cohesion: x, syntax: x, ...}"*）作为 user 输入，将 JSON 编码后的真实分数作为监督续写目标；训练时 prompt 部分用 `-100` 屏蔽，仅 JSON token 计入损失。LoRA（Hu et al. 2022）头作用在四个注意力投影上（`q_proj`、`k_proj`、`v_proj`、`o_proj`），秩 8、$\alpha=16$、dropout 0.05，可训练参数为底座 15.5 亿中的 218 万（占比 0.14%）。优化器 `adamw_torch`，学习率 $2 \times 10^{-4}$，cosine 调度 + 5% warmup，等效 batch size 8（每设备 2 + 梯度累积 4），最大序列长度 512。从 3911 篇训练样本中随机切出 10% 作为早停信号。

**结果。** 训练 3 个 epoch、wall-clock 约 7 小时 40 分；training loss 从 0.184 → 0.168 → 0.165 单调下降。validation cross-entropy 在该设备上被记录为 `nan`，是 fp16 + MPS 在稀疏 label mask 下的已知数值现象，不是模型本身的问题：(i) 我们仅以 training loss 选定最终 checkpoint，该指标稳定下降；(ii) 在 Kaggle 公共测试样本上的定性输出符合直觉（如样本 #1 → 3.0/2.5/3.0/2.5/2.5/2.5；样本 #3 → 3.5/4.0/3.5/3.5/3.5/3.5），后处理用正则提取 JSON 并就近吸附到官方网格 $\{1.0, 1.5, \dots, 5.0\}$。Kaggle code competition 的迟交不会反馈 MCRMSE，因此本 LLM 探针缺少一个可比较的总分。我们如实记录其状态：训练成功、推理可用、未量化评估。它的意义在于证明小型开源 LLM 可以在普通硬件上完成分维度作文评分的微调。

### 7.8 检索增强评分（DeBERTa-v3-base + KNN）

第二条 LLM/RAG 探针是免训练的检索式打分，位于 `RAG experiments/kaggle inference_rag_group-16.ipynb`，目的是看“仅靠检索相似训练样本”能走到哪儿。

**实验设置。** 冻结 **DeBERTa-v3-base** 编码器（He et al. 2023），用 attention mask 做 mean pooling 得到 768 维向量并 $L_2$ 归一化，再用 `sklearn.NearestNeighbors` 以 brute-force 余弦距离对全部 3911 篇训练样本建索引。每篇测试作文检索 $k=20$ 个最近邻，把余弦距离转换为相似度后乘以温度 10 再做 softmax 得到权重，预测分数为这 20 个邻居六维分数的加权平均。预测最终裁剪到 $[1.0, 5.0]$。整个流程不更新任何模型参数。

**结果。** 在 hold-out 验证集（取 `train.csv` 最后 500 篇，KNN 索引仅由前 3411 篇构建以避免泄漏）上，RAG 打分器达到 **0.5445 MCRMSE**。该数值落在单一 α Ridge（0.533）与 LightGBM SVD baseline（0.534）之间，弱于我们的传统堆叠（0.519）。这条探针的价值是它无需训练——超参数只有编码器选择、$k$ 与 softmax 温度——能给“任何学习模型”提供一个非常稳健的下界参考。它的缺陷与传统堆叠在 §7.5 暴露的相同：当邻居被中段作文主导时，预测会向语料均值收敛。

## 8. 传统方法与 LLM/RAG 方法的对比

课程要求明确讨论“LLM 与传统方法的对比”。结合 §7.7 与 §7.8 的实测，我们可以把这一节从“假设性比较”落到“实测比较”。

**Transformer 的优势，用一个数字说明。** 仅把冻结的 DeBERTa-v3-base 编码器当作检索索引、什么也不训练，就能在 500 篇 hold-out 上拿到 0.5445 MCRMSE。这意味着预训练编码器本身就编码了远超词袋的写作质量信号。文献中合理微调的 DeBERTa-v3-base 通常 0.46–0.47，DeBERTa-v3-large 0.44–0.45，提升集中在 grammar 与 conventions——也是我们目前损失最多的两列。

**LLM 的优势，定性说明。** Qwen2.5-1.5B + LoRA 是我们交付的形态最灵活的变体：它接受自然语言指令、返回结构化 JSON，可以在不动架构的前提下切换打分维度或细化 rubric。这让它非常适合作下游可编辑、可解释的评分组件。代价是约 7 小时的单 GPU 等效训练时间，以及把 1.5B 参数模型挂在演示后端的工程成本。对于评审需要在有限时间内复现的课程项目，这是一个真实的取舍，而不是免费午餐。

**传统堆叠仍然胜出在哪里。** 在唯一一致评估的指标——5 折分层 CV——下，0.51873 的传统堆叠依然领先两个 LLM/RAG 探针；它在笔记本 CPU 上约 4 分钟跑完，提供按目标列可审计的 leaderboard，并同时交付 CSV 与 Kaggle 推理 Notebook。第一名方案 0.43 是堆叠几十个 DeBERTa-v3-large + 伪标签的结果，量级上比我们多两个数量级的算力，不是 30% 课程项目的合理对照。

**RAG 与课程主题。** 经典 RAG 从外部知识库检索文本；我们这里的检索空间就是训练语料本身——它有时也被叫做 *neighbourhood-based prediction* 或 *KNN with neural embeddings*，是在“Kaggle code competition 推理时禁联网”约束下最直接、合规的 RAG 形式。我们没有再叠加合成 prompt-rubric 检索分支，因为公开 ELLIPSE 数据不含原始 prompt，合成检索分支不具备说服力。§7.8 的设置已经是数据本身能支撑的最强 RAG 变体。

**结论。** 精心实现的传统管线 0.519；冻结 DeBERTa 检索 0.544（零训练）；LoRA 微调的 1.5B LLM 在同一硬件预算内可以输出结构化分数；工业级 DeBERTa-large 集成需要数十倍算力才到 0.43。我们以传统堆叠作为头条提交，把 LLM/RAG Notebook 作为补充材料并附实测数据，绝不在报告中宣称未实测的指标。
...
- `GET /api/llm_rag` 返回 LoRA-Qwen 与 DeBERTa-RAG 实验摘要，对应页面上的“LLM and RAG”面板。
...
1. **三套验证机制不完全对齐。** 传统堆叠用 5 折分层 CV；RAG 用 500 篇 hold-out；LoRA-LLM 用 10% 随机 hold-out 且 eval-loss 在 fp16+MPS 下被破坏。这些数字之间不可直接比较，因此报告里把它们并排呈现，而不是塞进同一张 leaderboard。
2. **未在 stack 内跑 DeBERTa 监督微调。** RAG 探针仅把 DeBERTa 当作冻结检索器。若在同一 5 折划分下做有监督微调，预期可进入文献中报告的 0.46–0.47 区间，再叠加现有成分应能压到 0.46 或更低。瓶颈不在代码（`src/feedback_ell/transformer_model.py` 已就位），而在小组在项目周期内能稳定使用的 GPU 时间。
3. **未使用外部数据。** 我们仅使用比赛官方的 3911 篇作文。公开方案常借助上一届 Feedback Prize 数据预训练并使用伪标签，规则允许，但需要严谨的数据管理。
4. **演示预测器是启发式。** 部署的 `/api/predict` 使用确定性表层特征预测器，目的是让页面响应迅速；它不是训练后的堆叠模型。真正的堆叠模型只在离线脚本与 Kaggle 推理 Notebook 中调用。页面上展示的 leaderboard 数字是真实 OOF 结果。

后续优先级：先把 DeBERTa 监督微调接入同一 5 折 stack；其次引入上一届数据并谨慎做伪标签；最后把演示端的预测器替换为量化版 Ridge joblib（或 4-bit 量化的 Qwen2.5-1.5B），在保持低延迟的同时让前端展示的也是真实模型行为。

## 11. 结论

我们交付了一套完整、可复现的英语学习者作文分维度评分系统。基于 ELLIPSE 语料的 5 折交叉验证，按目标列堆叠的三成分模型达到 0.51873 MCRMSE，且全程仅依赖 CPU 友好、可解释友好的组件。在此基础上我们补充了两条契合课程主题的探针：对 Qwen2.5-1.5B-Instruct 做 LoRA 参数高效微调以输出结构化分数 JSON，以及免训练的 DeBERTa-v3-base + 余弦 KNN 检索式打分（500 篇 hold-out 上 0.5445 MCRMSE）。Web 演示、LLM/RAG Notebook、Kaggle 推理 Notebook 与 Docker 镜像构成完整交付物。模型仍最难处理的样本，恰好是有经验的人工评分者也最难判断的样本：分数分布的两端与最长篇幅作文，这也是下一轮工作的重点。

## 12. 团队分工

在补充 LLM 与 RAG 探针之后，小组对分工做了一次再平衡：LLM 微调与中英文报告同属"内容侧"工作，因此交给项目协调人统一推进；RAG 检索器以预训练编码器 + 检索为主，与数据/特征工程的工具栈天然契合，因此并入数据负责人；视频与 PDF 制作则由文献负责人与部署负责人联合承担——前者负责讲解稿与镜头脚本的把关，后者负责屏幕录制、剪辑和 PDF 排版。

| 成员 | 主要分工 | 贡献 |
| :--- | :--- | :--- |
| DHAYALA MURTHY SAVITHACCDS | 文献调研、问题定义与视频/PDF 制作 | 调研 Feedback Prize 公开方案与 AES 相关研究，整理 §3、§8 引用的 Qwen / LoRA / DeBERTa-v3 等参考文献，主导第 1、3、8 节，并联合负责最终展示视频与报告 PDF 的制作。 |
| HUANG ZIXUANCCDS | 数据审计、特征工程与 RAG 检索（§7.8） | 实现数据审计模块、十个文本统计量、TF-IDF + SVD 特征流水线以及 LLM/RAG 探针所需的验证切片；完成 §7.8 中免训练的 DeBERTa-v3-base + 余弦 KNN 检索式打分器。 |
| SUN MINGCCDS | 建模与评估 | 实现 MCRMSE、交叉验证框架、per-target Ridge / 融合 Ridge / LightGBM-SVD 三个传统成分模型，以及最终 CV MCRMSE 为 0.51873 的按目标列凸组合堆叠器。 |
| TAN XUAN ZHAOCCDS | 前端演示、部署与视频/PDF 制作 | 构建 FastAPI 服务、SVG 前端（含"LLM and RAG"面板）、Docker 镜像与阿里云部署；联合负责最终视频录制与报告 PDF 排版。 |
| WANG QIANCCDS | 报告、项目协调与 LLM 微调（§7.7） | 统筹小组工作，将实验结果整合为中英文报告，并完成 §7.7 中 Qwen2.5-1.5B-Instruct + LoRA 微调的训练、推理与吸附到官方 $\{1.0, 1.5, \dots, 5.0\}$ 网格的 JSON 后处理。 |
...
| `LLM experiments/local training_essay_scoring_m_chip.ipynb` | Qwen2.5-1.5B + LoRA 微调（训练） | §7.7 LLM 补充探针 |
| `LLM experiments/kaggle inference_group-16.ipynb` | Qwen2.5-1.5B + LoRA 微调（推理） | §7.7 LLM 补充探针 |
| `RAG experiments/kaggle inference_rag_group-16.ipynb` | DeBERTa-v3-base + 余弦 KNN RAG | §7.8 RAG 补充探针 |
